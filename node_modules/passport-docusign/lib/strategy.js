 ...
 *         callbackURL: 'https://www.youdomain.com/auth/docusign/callback',
 *         production: true
 *       },
...
function Strategy(options, verify) {
  ...
  this._baseURL = options.production ? 'https://account.docusign.com/oauth' : 'https://account-d.docusign.com/oauth';
  ...
  this._oauth2.get(url, accessToken, function (err, body, res) {
    ...
    if (!body && err) {
      ...
      if (json && json.error && typeof json.error === 'object') {
        return done(new docusignAPIError(json.error.message, json.error.type, json.error.code, json.error.error_subcode, json.error.errorCode));
      } else if (json && json.error && typeof json.error === 'string') {
        return done(new docusignAPIError(json.error));
      }
...
Strategy.prototype.parseErrorResponse = function (body, status) {
  ...
  if (json.error && typeof json.error === 'object') {
    return new docusignTokenError(json.error.message, json.error.type, json.error.code, json.error.error_subcode, json.error.errorCode);
  } else if (json && json.error && typeof json.error === 'string') {
    return new docusignTokenError(json.error_description, undefined, json.error, undefined, undefined, status);
  }
